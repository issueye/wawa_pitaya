...
func RSADecryptAndSalt(account, pwd, privateKey string) (newPwd string, err error) {
	decrypt, err := RSADecrypt(Strings{Str: pwd}.StringToByteSlice(), Strings{Str: privateKey}.StringToByteSlice())
	if err != nil {
		return
	}
	newPwd = EncodeMD5(Strings{}.ByteSliceToString(decrypt) + account)
	return
}
